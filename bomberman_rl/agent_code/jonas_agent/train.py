    ...
    for action in Actions:
        ...
        if np.sum(action_indices) > 0:   
            ...
            if allFit(self):
                ...
                predicted_future_reward = np.max([self.trees[tree].predict(new_features) for tree in self.trees])
                predicted_action_value = self.trees[action.name].predict(old_features)

                q_values = current_guess + LEARNING_RATE * ( actual_rewards + DISCOUNT * predicted_future_reward - predicted_action_value)
...
def state_to_features(self, game_state):
    if(game_state is None): return np.array([[ 0],
                                    [ 0],
                                    [ 0],
                                    [ 0],
                                    [ 1],
   ...
   # print("ACTION TO NEAREST COIN", actionToNearestCoin)
    features = np.array([[ int(actionToNearestCoin == "UP")],
