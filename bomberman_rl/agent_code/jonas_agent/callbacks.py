...
def act(self, game_state):
    ...
    """
    if allFit(self):
        if game_state["round"] < 1000: rand = 1
        else: rand = 0
        if(rand < 0.5):
            ...
            winner_index = np.argmax([self.trees[tree].predict(features[Actions[tree].value].reshape(-1, 1)) for tree in self.trees])
            self.next_action = Actions(winner_index).name
        else:
            self.next_action = np.random.choice(['RIGHT', 'LEFT', 'UP', 'DOWN', 'BOMB', 'WAIT'])
