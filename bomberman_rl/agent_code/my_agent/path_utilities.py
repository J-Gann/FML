...
class FeatureExtraction:
            ...
            for y in range(s.ROWS):
                ...
                for nx, ny in neighbors:
                    if 0 <= nx < s.COLS and 0 <= ny < s.ROWS:
                        ...
                        if not self._node_obstructed(
                            node
                        ) and not self._node_obstructed(neighbor):
                            row.append(node)
                            col.append(neighbor)
                            data.append(1)
    ...
    def _to_node(self, index):
        return index[1] * s.COLS + index[0]

    def _to_index(self, node):
        return (node % s.COLS, math.floor(node / s.COLS))

    def _remove_obstructed_nodes(self, nodes):
        free_nodes = []
        for node in nodes:
            if not self._node_obstructed(node):
    ...
    def FEATURE_move_to_nearest_coin(self):
        return self._next_step_to_nearest_index(self.coins)
    ...
    def FEATURE_boxes_in_agent_blast_range(self):
        ...
        for i in range(4):
            if 0 < x + i < s.COLS and self.field[x + i, y] == 1:
                sum += 1
            if s.COLS > x - i > 0 and self.field[x - i, y] == 1:
                sum += 1
            if 0 < y + i < s.ROWS and self.field[x, y + i] == 1:
                sum += 1
            if s.ROWS > y - i > 0 and self.field[x, y - i] == 1:
                sum += 1
