import numpy as np
from agent_code.null_agent.features.feature import FeatureCollector, MoveToNearestCoin
...
def setup(self):
    ...
    self.feature_collector = FeatureCollector(MoveToNearestCoin())
...
def explore(self):
    """Author: Jonas Gann"""
    choice = np.random.choice(list(Actions), p=[0.18, 0.18, 0.18, 0.18, 0.18, 0.1])  # Randomly perform an action
...
def exploit(self, game_state):
    ...
    for (
        action
    ) in Actions:  # Compute the predicted reward for each action and execute the one with the largest predicted value
        action = action.name
